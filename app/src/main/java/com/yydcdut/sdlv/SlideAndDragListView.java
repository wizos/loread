...
//        mState = STATE_LONG_CLICK; // FIXME: 2016/5/4 【添加】修复他的 长按 bug
        switch (msg.what) {
...
//                        mVibrator.vibrate(100); // 触发震动
                        ...
                        mState = STATE_NOTHING; // 在实际的操作中，这一步基本没有太大作用。因为后续 还会触发 MOVE 导致变为 STATE_LONG_CLICK
                    }
...
//                if(mState == STATE_NOTHING){return true;} // 防止在处理 long click 时 已被置为 nothing 的 mState 在下面被改为 long click 导致又触发 Action up 中的点击事件
                if ( fingerNotMove(ev) && mState != STATE_SCROLL && mState != STATE_NOTHING) {//手指的范围在50以内
//                    sendLongClickMessage(pointToPosition(mXDown, mYDown));
    ...
    public interface OnSlideListener {
         ...
         * @param position
         */
        int onSlideOpen(View view, View parentView, int position, int direction);  // FIXME: 2016/5/4 【实现划开自动返回】把返回类型由 void 改为 int
    ...
    @Override
    public int onSlideOpen(View view, int position, int direction) { // FIXME: 2016/5/4 【实现划开自动返回】把返回类型由 void 改为 int
        if (mOnSlideListener != null) {
            return mOnSlideListener.onSlideOpen(view, this, position, direction);
        }
        return Menu.ITEM_NOTHING; // FIXME: 2016/5/4 【实现划开自动返回】多加的
    }

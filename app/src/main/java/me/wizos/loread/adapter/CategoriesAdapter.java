...
public class CategoriesAdapter extends ExpandableAdapter<RecyclerView.ViewHolder> implements StickyCreator { // implements StickyCreator
    ...
    static class ParentHolder extends RecyclerView.ViewHolder {
        ...
        public void bind(@NonNull CategoriesAdapter mAdapter, @NonNull Collection category, final int parentPosition) {
            adapter = mAdapter;
            if(category.getId().contains(App.CATEGORY_UNCATEGORIZED)){
                if(CoreDB.i().feedDao().getFeedsCountByUnCategory(App.i().getUser().getId()) == 0){

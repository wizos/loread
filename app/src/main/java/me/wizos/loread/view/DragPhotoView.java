...
public class DragPhotoView extends PhotoView {
    ...
    public DragPhotoView(Context context, AttributeSet attr, int defStyle) {
        ...
        tapHelper = new TapHelper(this, new TapHelper.OnTapListener() {
            @Override
            public void onLongTap(DragPhotoView view) {
                if (mTapListener != null) {
                    mTapListener.onLongTap(view);
                }
            }
//            @Override
//            public void onTap(DragPhotoView view) {
//                if(mTapListener!=null){
//                    mTapListener.onTap(view);
//                }
//            }
        });
    }
        ...
        if (getScale() == 1) {
            ...
            switch (event.getAction()) {
                case MotionEvent.ACTION_DOWN:
                    ...
                    canFinish = !canFinish;
                    // 添加
//                    tapHelper.sendLongClickMessage(event);
                    break;
                // 这个是实现多点的关键，当屏幕检测到有多个手指同时按下之后，就触发了这个事件
                case MotionEvent.ACTION_POINTER_DOWN:
//                    tapHelper.removeLongClickMessage();
//                mState = STATE_MORE_FINGERS;
                    //消耗掉，不传递下去了
                    return true;
                case MotionEvent.ACTION_MOVE:
//                    tapHelper.move(event);
                    //in viewpager

...
public class TinyRSSTokenInterceptor implements Interceptor {
    @NotNull
    @Override
    public Response intercept(Chain chain) throws IOException {
        ...
        if (BuildConfig.DEBUG) {
            if (!TextUtils.isEmpty(bodyString)) {
                ...
                XLog.d("body 为空----------> " + originalResponse.code() );
            }
        }
        ...
        if (bodyString.contains("\"error\":\"NOT_LOGGED_IN")) {
            ...
            loginResult = ((TinyRSSApi)App.i().getApi()).login(user.getUserId(),user.getUserPassword());

            if( loginResult.isSuccess() ){
                XLog.v("TokenInterceptor授权过期：成功重新登录 " + loginResult.getData() );
